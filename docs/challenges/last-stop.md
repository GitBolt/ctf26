...
Solana does not encode the boundary between adjacent PDA seed byte strings. The route `redterminus` therefore collides with line `red` plus station `terminus`. Inspection reveals the one-field and two-field PDA layouts without printing the concatenated answer; the station itself supplies the field values. The winning card is an ordinary kiosk product whose address is accepted as the restricted gate card.
...
- Stop-only autonomous-agent policy with no participant-visible reporting mechanism.
