...
3. Recover the second signer from the physical card issued at the venue, or its archived public-practice copy, and co-sign `unlock_second`.
...
**Lock two was venue-local during the event** (§4.3). The hidden key was never served over the network; its 64 bytes were Base64-encoded into a compact NDEF Text record on an NTAG213-compatible plain PVC card handed out at the desk. Participants read the card with NFC Tools or another NDEF reader, copied the Text value, and decoded it locally. This was a *pace* gate rather than a per-participant gate: one shared key served the whole field, so sharing remained possible after the first physical discovery. The post-event public deployment exposes the retired card payload after launch so the same mechanics remain solvable without venue hardware.
...
The event used the prototype program at `Ekw4Zx3Nu9zTvCYsuzn1ubHNtgWjRAtm8PMUNavgmPXj`. Its keypairs are absent from the public tree. The retired card is supplied only through public-practice deployment configuration. A fresh scored run must generate new challenge-scoped keys, update the program constants, redeploy, and omit `THE_CHAMBER_PUBLIC_CARD_BASE64` before issuing new NFC cards.
