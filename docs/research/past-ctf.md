...
## 12. Real-World CTF Research Patterns To Steal

The next round of challenge ideas should feel closer to real CTFs and bug bounty labs:
...
- source archaeology
- visual/audio clues
- social/web navigation
- only then a real Solana/Arcium security action

Research patterns:

- DEFCON Bug Bounty Village `GeneQuest` worked because it felt like a real startup, with product pages, dashboards, research portals, 10 microservices, 80+ endpoints, and multi-step chains across distant sources/sinks.
- BountyCon `Tick-Tock` was a white-box AWS Lambda/pickle challenge where exploitation required understanding runtime constraints, not just grepping a flag.
- BountyCon `Lighthouse` converted native-code output to phone flashlight Morse; the solver recorded video and used tooling to decode the temporal signal.
- DIVER OSINT `air2air` used a video, aircraft logo, seat position, flight direction, date, aircraft type, and flight tracking to identify a plane. This is exactly the kind of visual/context navigation agents struggle with under normal tooling.
- NahamCon-style media tasks show that unusual visual encodings and app-specific scanners can force tool selection and manual validation.
- Paradigm/idek Solana writeups show good real bug classes, but many are still scriptable once context is given. We should use those bug classes as final exploit steps, not as the whole challenge.

Design correction:

> Off-chain investigation should identify which Solana object, commit, wallet, route, or authority matters. The final flag still requires an on-chain exploit/state transition.

Do not make all challenges:

```text
read program -> spot vuln -> write script -> submit tx
```

Prefer:

```text
investigate messy real-world trail -> identify live target/context -> exploit real Solana bug -> checker validates state
```

### Five Better Hybrid Challenge Ideas

#### 1. Sponsor Receipt Trail

Start:

- event sponsor page
- fake booth photos
- sponsor staff X/Twitter profile
- one photo shows a tablet with a partial Solana address/window number
- an AI-generated flashy dashboard image is a red herring

Human investigation:

- identify the real sponsor staff account
- choose the photo that matches event date/location
- recover enough of the address/window to find the receipt account
- distinguish dashboard receipt from sponsor-signed receipt

Security core:

- CPI authority confusion or receipt-program pinning bug
- attacker deploys fake receipt program and settles against a per-team vault

Anti-AI:

- visual partial address
- multiple plausible sponsor profiles
- per-team canary token in fake dashboard export
- final exploit requires fresh team-bound tx

#### 2. The Aircraft Wallet

Start:

- social account with many posts
- one post has aircraft photo from event travel
- caption hints at an auditor or sponsor arriving late
- another post contains an attractive fake restaurant/geolocation sink

Human investigation:

- read context and avoid the fake restaurant sink
- identify aircraft/tail/flight from image clues
- map flight/operator to sponsor entity
- find sponsor wallet from public bounty/sponsor payment trail
- inspect that wallet's interaction with the vulnerable Solana program

Security core:

- stale escrow route or missing signer/mint validation
- solver must exploit the same bug against their per-team escrow

Anti-AI:

- no clean API for the full chain
- visual aircraft clues
- external navigation and judgment
- per-team decoy phrase in the fake restaurant post

#### 3. Patch Window

Start:

- challenge says: "deployed from the week before the fix"
- players get a repo, a release note, and an event dashboard
- multiple commits look security-relevant

Human investigation:

- browse PRs/issues/commit history
- find the actual fix, not the loud changelog item
- map fixed code to deployed program hash or IDL behavior
- derive the exploit condition from the diff

Security core:

- missing account validation, non-canonical PDA, or fake token program
- exploit must mutate team-specific state

Anti-AI:

- large repo and misleading security advisory
- source screenshot/terminal video with one important line
- fake "autosolver patch notes" canary
- final checker validates state, not answer text

#### 4. Conference Clip Leak

Start:

- 60-second fake conference clip
- speaker mentions three project names
- slide flashes PDA seed/account namespace briefly
- audio says a negation or correction
- background terminal shows the real program ID

Human investigation:

- combine audio and video timing
- identify which flashed clue is negated
- map project name to deployed program/version
- recover target PDA namespace

Security core:

- PDA seed collision or non-canonical bump exploit
- solver creates a counterfeit account in the selected namespace

Anti-AI:

- screenshot alone is insufficient
- transcript alone is wrong
- per-session generated clip can bind clue to nonce
- fake transcript contains canary token

#### 5. Wallet Support Ticket

Start:

- support-ticket bundle:
  screenshots of failed wallet tx
  explorer screenshot
  Discord support export
  fake helper script
  short video of a failed transaction

Human investigation:

- reconstruct the real account order from visuals
- identify which support advice is wrong
- notice ALT/v0 transaction behavior from explorer screenshot
- map the route to a vulnerable router

Security core:

- Address Lookup Table account substitution
- quote signs account indexes but not resolved pubkeys
- solver crafts v0 tx using team ALT

Anti-AI:

- mixed screenshots/video/text
- fake helper compiles but targets stale route
- per-team support ticket watermark
- final proof requires team-bound route exploit

### Design Rule For These Hybrids

Each challenge should have:

1. identity-bound artifact or launch ticket
2. messy human investigation layer
3. one or two attention-sink red herrings
4. per-team canary token in artifacts
5. real Solana/Arcium exploit or state transition
6. checker-generated flag only after solve
