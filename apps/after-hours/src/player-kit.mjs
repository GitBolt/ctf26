...
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../player-kit");
const FILES = Object.freeze(["README.md", "checkout.mjs", "package.json"]);
let cachedArchive;

export function playerKitArchive() {
  if (cachedArchive) return cachedArchive;
  cachedArchive = zip(FILES.map((name) => ({
    name: `after-hours-player/${name}`,
