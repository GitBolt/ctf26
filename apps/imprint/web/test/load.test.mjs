...
function request(ip = "203.0.113.20") {
  return new Request("https://imprint.example/api/test", {
...
test("IMPRINT session budget accepts 50 participants and rejects a sustained source flood", async () => {
  ...
  await Promise.all(
    Array.from({ length: 50 }, () =>
      consumeImprintRequestBudget("session", {
        request: request(),
        env: ENV,
        execute,
      })
  ...
  for (let index = 0; index < 70; index += 1) {
    await consumeImprintRequestBudget("session", {
      request: request(),
      env: ENV,
      execute,
    });
...
test("weighted RPC budgets bound expensive batches without coupling identities", async () => {
  ...
  const participants = Array.from(
    { length: 50 },
    (_, index) => `rpc-player-${index + 1}`
  );
  const lightCalls = [{ method: "getAccountInfo" }];
  const heavyCalls = Array.from({ length: 10 }, () => ({
  ...
  await Promise.all(
    participants.map((participantId, index) =>
      consumeImprintRequestBudget("rpc", {
