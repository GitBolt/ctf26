...
import { credentialForTeam, parseCredentialRoster } from "../lib/credential-roster.mjs";
...
function entry(teamId, secretByte) {
  ...
  return {
    ...
    credentialPublicKeyCoseBase64: coseP256PublicKey(publicKey).toString("base64url"),
    counter: 0,
    transports: ["usb"],
  };
}
...
test("derives the on-chain P-256 key exclusively from an organizer roster COSE key", () => {
  ...
  assert.match(roster[0].passkeyPubkey.toString("hex").slice(0, 2), /^(02|03)$/);
});

test("requires one unique, valid physical-key record for every team", () => {
  const first = entry("team-a", 1);
  assert.throws(
    () => parseCredentialRoster(JSON.stringify([first, { ...first, credentialId: "credential-2" }])),
    /teamId is invalid or duplicated/,
  );
  assert.throws(
    () => parseCredentialRoster(JSON.stringify([{ ...first, credentialPublicKeyCoseBase64: "not base64!" }])),
    /must be base64url/,
  );
  ...
  assert.throws(
    () => credentialForTeam("team-b", { IMPRINT_CREDENTIAL_ROSTER_JSON: JSON.stringify([first]) }),
