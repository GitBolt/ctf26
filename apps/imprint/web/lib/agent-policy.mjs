...
export async function policyResponse() {
  try {
    ...
    await recordImprintIntegrity(current, "policy-read", "policy", { headers: requestHeaders }, "policy");
...
export async function disclose(request) {
  try {
    ...
    const requestHeaders = await headers();
    const result = await forwardDisclosure({ identity: { ...current, eventId: current.eventId }, challenge: "imprint", label: "IMPRINT", agent: body.agent, model: body.model, requestMeta: { userAgent: requestHeaders.get("user-agent") || "" } });
