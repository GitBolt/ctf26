...
export const CLAIM_TITLE = "$ST Genesis Airdrop";
...
export function claimMessage(teamId, nonce) {
  return [
    CLAIM_TITLE,
...
export function createInstance(teamId, options = {}) {
  const random = options.randomInt || crypto.randomInt;
  const nonce = options.nonce || crypto.randomBytes(12).toString("base64url");
  return {
    version: 1,
    teamId,
    nonce,
    message: claimMessage(teamId, nonce),
    target: options.target || random(MIN_TARGET, MAX_TARGET + 1),
