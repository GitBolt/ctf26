...
declare_id!("HzXXcU1Jvy2c8twFMAwjCKbX3A43sbLwCbYKaTk47KPE");
...
pub mod quarry_vault_fixed {
    ...
    pub fn execute_strategy(ctx: Context<ExecuteStrategy>, amount: u64) -> Result<()> {
        ...
        require_keys_eq!(
            ...
            ctx.accounts.vault.pinned_strategy_program,
            QuarryError::InvalidStrategyProgram
        );
...
pub struct ExecuteStrategy<'info> {
    ...
    pub vault: Account<'info, Vault>,
    /// CHECK: validated as this program's PDA and forwarded only to the pinned strategy.
    #[account(
    ...
    /// CHECK: executable and pinned to the value stored in the vault.
    #[account(executable)]
...
pub enum QuarryError {
    ...
    InvalidAmount,
    #[msg("strategy program is not the vault's pinned strategy")]
