...
import { DEFAULT_TARGET, DECOY_VIDEO_IDS, WINNING_VIDEO_ID, createInstance, decodeReceiptRecord, ordinaryReceipt, parseAndVerifyClaim, recordClaim, solveReceipt, videoId } from "../src/protocol.mjs";
...
test("every participant receives the same event-wide claim threshold", () => {
  assert.equal(createInstance("participant-a").target, DEFAULT_TARGET);
  assert.equal(createInstance("participant-b").target, DEFAULT_TARGET);
  assert.equal(DEFAULT_TARGET, 8);
});

test("one Solana wallet authorization produces fifteen accepted byte-distinct variants", () => {
  const instance = createInstance("participant-crypto", { target: 15, nonce: "fixed-instance", now: 1 });
...
test("the solved receipt uses the same video record shape as every decoy", () => {
  const instance = createInstance("participant-receipt", { target: 1, nonce: "fixed", now: 1 });
  const wallet = nacl.sign.keyPair();
  const signature = nacl.sign.detached(Buffer.from(instance.message), wallet.secretKey);
  ...
  const retry = recordClaim(result.instance, verified, { randomByte: 0, now: 3 });
  assert.equal(retry.completed, true);
  assert.equal(retry.duplicate, true);
  assert.equal(retry.receipt, result.receipt);
});
