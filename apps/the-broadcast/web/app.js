...
async function event(name) { try { await request("/api/ui-event", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ event: name }) }); } catch {} }
...
async function load() {
  try {
    ...
    await event("page-ready");
    if (navigator.webdriver) await event("automation-present");
    $("message").textContent = state.config.message;
    if (state.config.completed) setStatus("This participant instance is already complete.", "success");
  } catch (error) { setStatus(error.message, "error"); }
}

async function connect() {
  void event("wallet-connect");
