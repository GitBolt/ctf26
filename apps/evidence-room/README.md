...
Public Devnet remains a shared, observable environment. An outside party can watch factory transactions and race or disrupt newly revealed accounts. The bounded credit prevents a small amount of interference from penalizing a participant, but it cannot eliminate sustained denial of service. A battle-hardened event should use an isolated validator or an access-controlled RPC and validator environment for this challenge. Redis-backed participant and capacity leases keep expensive work bounded across service replicas.
...
Production also requires `EVIDENCE_ROOM_EXPECTED_PARTICIPANTS`. Readiness derives the live mint, token-account, and system-account rent, estimates the full four-case budget for every expected participant, applies `EVIDENCE_ROOM_CAPACITY_BUFFER_BPS`, and fails closed when the factory payer cannot cover that amount. At the rent values pinned in the regression test, the estimate is 57,185,760 lamports per participant before the default 10 percent reserve buffer.
