...
test("50 concurrent passages preserve participant identity and reject spam", async (t) => {
  ...
  assert.equal(new Set(launches.map(({ body }) => body.password)).size, 50);
  ...
  const slots = await Promise.all(participants.map((participantId) => service.store.acquireSlot("load-validator", participantId, 1_000, 10_000, 24)));
  assert.equal(slots.filter(Boolean).length, 24);
  assert.equal(await service.store.acquireSlot("load-validator", participants[0], 1_001, 10_000, 24), false);
